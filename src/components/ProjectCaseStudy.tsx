              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-medium text-gray-900">{project.timeline || 'N/A'}</p>
                  </div>

                  {project.teamSize && (
                    <div>
                      <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2">{t('teamSize') || 'Team Size'}</h3>
                      <p className="text-lg font-medium text-gray-900">{project.teamSize}</p>
                    </div>
                  )}

                  <div>
                    <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2">{t('servicesWeProvided')}</h3>
                    <div className={`flex flex-wrap gap-2 mt-2 ${isRTL ? 'justify-start' : ''}`}>
                      {(project.services || (project.technologies && project.technologies.slice(0, 3)) || []).map((service, index) => (
                ...
                <div className="pt-6 border-t border-gray-200">
                  ...
                  <div className={`flex ${isRTL ? 'flex-row space-x-reverse' : 'space-x-3'}`}>
          ...
          <div className="lg:col-span-3">
            ...
            {project.description && (
              <div className={`mb-12 ${isRTL ? 'text-right' : ''}`}>
                <p className="text-xl text-gray-700 leading-relaxed" dir={isRTL ? 'rtl' : 'ltr'}>
                  {project.description}
                </p>
              </div>
            )}
            ...
            {/* Challenge Section */}
            {project.challenge && (
              <section className={`mb-16 ${isRTL ? 'text-right' : ''}`}>
                <h2 className={`text-3xl font-bold text-gray-900 mb-6 ${isRTL ? 'text-right' : ''}`} dir={isRTL ? 'rtl' : 'ltr'}>
            ...
            {project.technologies && project.technologies.length > 0 && (
              <section className={`mb-16 ${isRTL ? 'text-right' : ''}`}>
                <h2 className={`text-3xl font-bold text-gray-900 mb-8 ${isRTL ? 'text-right' : ''}`} dir={isRTL ? 'rtl' : 'ltr'}>
                  {t('technologies') || 'Technologies Used'}
                </h2>
                <div className="grid md:grid-cols-2 gap-6">
                  {project.technologies.map((tech, index) => (
                    <div key={index} className="bg-gray-50 rounded-xl p-6">
                      <h3 className={`text-xl font-semibold text-gray-900 mb-2 ${isRTL ? 'text-right' : ''}`} dir={isRTL ? 'rtl' : 'ltr'}>
                        {typeof tech === 'string' ? tech : tech.name}
                      </h3>
                      {typeof tech === 'object' && tech.description && (
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {project.results.map((result, index) => (
                    <div key={index} className="bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl p-6 text-center">
                      <div className="text-4xl font-bold text-[#6812F7] mb-2">
                        {typeof result === 'string' ? result : result.metric}
                      </div>
                      {typeof result === 'object' && result.description && (
                        <p className={`text-gray-700 text-sm ${isRTL ? 'text-right' : ''}`} dir={isRTL ? 'rtl' : 'ltr'}>
                          {result.description}
                        </p>
                      )}
                    </div>
            ...
            {project.clientTestimonial && (
              <section className="py-20 bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 rounded-3xl mb-12 relative overflow-hidden">
                ...
                <div className="relative z-10 text-center max-w-5xl mx-auto px-6">
                  ...
                  <div className={`flex flex-col sm:flex-row items-center justify-center space-y-4 sm:space-y-0 ${isRTL ? 'sm:flex-row sm:space-x sm:space-x-6' : 'sm:space-x-6'}`}>
