...
class EmailService {
  ...
  private generateContactEmailHTML(contactMessage: ContactMessage): string {
    const { name, email, phone, company, message, service, budget, createdAt } = contactMessage;
      ...
      <body>
        <div class="container">
          ...
          <div class="footer">
            ...
            ${email ? `<p>Please respond directly to the customer's email address: <a href="mailto:${email}">${email}</a></p>` : ''}
            ${phone && !email ? `<p>Please contact the customer by phone: <a href="tel:${phone}">${phone}</a></p>` : ''}
            ${email && phone ? `<p>Contact: <a href="mailto:${email}">${email}</a> or <a href="tel:${phone}">${phone}</a></p>` : ''}
          </div>
        </div>
      </body>
      </html>
...
${email ? `Please respond directly to the customer's email address: ${email}` : ''}
${phone && !email ? `Please contact the customer by phone: ${phone}` : ''}
${email && phone ? `Contact: ${email} or ${phone}` : ''}
    `.trim();
  }
